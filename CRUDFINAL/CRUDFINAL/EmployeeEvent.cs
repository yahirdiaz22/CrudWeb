...
namespace CRUD
{
    public partial class EmployeeEvent : Form
    {
        ...
        private void loadEmployee()
        {
            ...
            {
                connection.Open();
                string query = "SELECT EmployeeEvent.idEmployeeEvent, Employee.name AS EmployeeName, Event.name AS EventName, EmployeeEvent.status AS EmployeeEventStatus FROM EmployeeEvent INNER JOIN Employee ON EmployeeEvent.idEmployee = Employee.idEmployee INNER JOIN Event ON EmployeeEvent.idEvent = Event.idEvent WHERE  EmployeeEvent.status = 1;\r\n";
            ...
            {
                connection.Open();
                string query = "SELECT idEmployee, name FROM Employee WHERE status =1";
                SqlDataAdapter adapter = new SqlDataAdapter(query, connection);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                connection.Open();
                string query = "INSERT INTO EmployeeEvent ( idEmployee, idEvent, status) " +
        ...
        private void LoadDataFromGrid()
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                comboBox1.Text = selectedRow.Cells["Employeename"].Value.ToString();
                comboBox2.Text = selectedRow.Cells["Eventname"].Value.ToString();
                chkStatus.Checked = Convert.ToBoolean(selectedRow.Cells["EmployeeEventStatus"].Value);
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                int departmentId = Convert.ToInt32(dataGridView1.SelectedRows[0].Cells["idEmployeeEvent"].Value);
                string ReportInfo = comboBox1.SelectedItem.ToString();
                int idStudent = 0;
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();
                        string query = "UPDATE EmployeeEvent SET idEmployee  = @idContract, idEvent  = @idPosition, status = @status " +
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                {
                    connection.Open();
                    string query = "Update EmployeeEvent set status = 0 WHERE idEmployee = @idEmployeeEvent";
